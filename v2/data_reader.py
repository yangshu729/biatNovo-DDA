...
from DataProcess.deepnovo_cython_modules import process_spectrum,get_candidate_intensity_dda
...
class DeepNovoTrainDataset(Dataset):
    def __init__(self, feature_filename, spectrum_filename, extra_denovo_filename = None, transform=None):
        ...
        if os.path.exists(spectrum_location_file):
            ...
            logger.info(f"WorkerIO: read cached spectrum locations from {spectrum_location_file}")
    ...
    def _get_spectrum(self, feature : DIAFeature, input_spectrum_file_handle):
        ...
        for i, id in enumerate(peptide_ids_forward[:-1]):
            ...
            candidate_intensity = get_candidate_intensity_dda(spectrum_original_forward, feature.precursor_mass, prefix_mass, 0)
        ...
        for i, id in enumerate(peptide_ids_backward[:-1]):
            ...
            candidate_intensity = get_candidate_intensity_dda(spectrum_original_backward, feature.precursor_mass, suffix_mass, 1)
    ...
    def _parse_spectrum(self, precursor_mz, precursor_mass, rt_mean, scan_list, ms1_list, input_file_handle):
        ...
        for scan, ms1_intensity in zip(scan_list_middle, ms1_intensity_list_middle):
            ...
            assert "BEGIN IONS" in line, f"Error: wrong input, 'BEGIN IONS' not found. Last SCAN value: {scan}, line: {line}, spectrum_location: {spectrum_location}"
            line = input_file_handle.readline()
